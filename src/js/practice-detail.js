...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const practiceId = urlParams.get("id"); // récupère l'id de l'URL
    ...
    const practices = {
        ...
        "stay-calm": {
            ...
            tips: ["Respirez profondément", "Ne paniquez pas", "Relisez vos réponses si nécessaire"]
        },
        "prioritize": {
            title: "Prioriser les questions faciles",
            category: "Stratégie",
            description: "Commencez par les questions que vous maîtrisez pour gagner du temps",
            tips: ["Répondez aux questions faciles d'abord", "Laissez les difficiles pour la fin"]
        }
    };
    ...
    // Mettre à jour les conseils pratiques
    const tipsSections = document.querySelectorAll(".numbered-list");
    tipsSections.forEach(section => section.innerHTML = "");
    practice.tips.forEach((tip, i) => {
        const li = document.createElement("li");
        li.textContent = tip;
        tipsSections[0].appendChild(li);
    });

    /* ===================== CONFIG ===================== */
const PRACTICE_ID = practiceId; // <-- utiliser l'id passé dans l'URL
const TOTAL_PRACTICES = 10;
    ...
    function updateProgression() {
        ...
        const inProgress = Math.min(2, followed); // logique simple
    ...
    function saveFollowedPractices() {
        localStorage.setItem(
            ...
            JSON.stringify(followedPractices)
        );
    }
